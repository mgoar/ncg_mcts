...
n_ = np.hstack(
    [np.arange(10, 15), np.logspace(np.log10(15), np.log10(100), 10).astype(int)[1:]])
sparse = False
regular = False

for n in n_:
    alphas = np.hstack(
        [n, np.logspace(np.log10(n), np.log10(2*n), 10, dtype=float)[1:]])

    # Logging config
    format = "%(asctime)s: %(message)s"
    logging.basicConfig(filename='log_random_'+str(n)+'.txt',
